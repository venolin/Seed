...
var myVar = setInterval(unit,1000); //Remember this is an asynchronous operation. Asynchronous operations are only processed after the entire page (all the synchronous operations ave already completed. This would also suggest why it takes more than one page reload to display the generation of units after a link is clicked. Can't write it unit() or with a value like so unit(test) as it will stop running every  time interval and only run once in order to return a value.
...
<script>
document.writeln(bool_if_result);
if(bool_if_result==false){
